...
export default function TheatreV1Layout({ project, others, stackLinks }: ProjectLayoutProps) {
  const asideRef = useRef<HTMLElement | null>(null);
  const [tapestryHeight, setTapestryHeight] = useState<number>(260);
  const [activeImage, setActiveImage] = useState<{ src: string; alt: string } | null>(null);
  const categoryLabel = formatCategoryLabel(project.category);
  const categoryDetailEntries = useMemo(
    () => resolveLayoutDetailEntries('theatre_v1', project.categoryMeta, project.entryLines),
    [project.categoryMeta, project.entryLines],
  );
  ...
  }) as CSSProperties, [tapestryHeight]);
  ...
  useEffect(() => {
    ...
    const syncHeight = () => {
      ...
      setTapestryHeight(Math.max(180, Math.round(panelHeight * 0.5)));
    };
  ...
  const detailAside = (
    <aside ref={asideRef} className="space-y-4 p-6 rounded-xl border border-white/10 bg-white/5">
  ...
  return (
    <div className="space-y-6" data-project-layout="theatre_v1">
      ...
      <div className="grid lg:grid-cols-3 gap-10 items-start">
        <div className="lg:col-span-2 space-y-8">
          {mergedMedia.length > 0 ? (
            <div className="space-y-4" data-mwb-highlight-id="gallery">
              <h3 className="text-xl font-semibold">Media</h3>
              <div
                className="media-ribbon h-64 sm:h-72 lg:h-[var(--tapestry-height)] overflow-x-auto overflow-y-hidden pb-2 scroll-smooth"
                style={tapestryStyle}
              >
                {ribbonItems}
              </div>
